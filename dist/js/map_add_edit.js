...
var addEditMap = function () {
    var map;
    var mordplatsen = { lat: 59.336615, lng: 18.062775 };
    ...
    var defaultSettings = function defset() {
        //self invoking but can also be called, activates default = observation
        activeBox = obsBox;
    ...
    function initMap() {
        ...
        // Static marker
        var murderMarker = new google.maps.Marker({
            map: map,
        ...
        var residence = new google.maps.Marker({
            map: map,
        ...
        google.maps.event.addListenerOnce(map, 'idle', function () {
            if (activeBox.lat.value && activeBox.lng.value) {
                //If page loads with something in coords-fields --> display on map
    ...
    function placeMarker(location, map, pan) {
        //Should have option to not always effect active marker (set position btn)
        if ((typeof activeMarker === 'undefined' ? 'undefined' : _typeof(activeMarker)) === 'object') {
            //if we have a marker, move it 
            activeMarker.setPosition(location);
        } else {
            //if we don't have a marker, place it
    ...
    function activateMarker(target) {
        ...
        if (!activeMarker.moved) {
            //if we haven't moved marker previously...
            if (activeBox.lat.value && activeBox.lng.value) {
                //check if we're maybe on the edit page and there's an inherited value
    ...
    function getAdress() {
        if (!obsBox.lat.value || !obsBox.lng.value) {
            ...
            geoCode('http://maps.googleapis.com/maps/api/geocode/json?latlng=' + obsBox.lat.value + ',' + obsBox.lng.value, function (response) {
                var street = response.results["0"].address_components[1].long_name;
                var streetno = response.results["0"].address_components["0"].long_name;
    ...
    function getCoords() {
        if (!adressbox.value) {
            ...
            var adress = adressbox.value + ", Stockholm, Sverige";
            geoCode('http://maps.google.com/maps/api/geocode/json?address=' + adress, function (response) {
