...
var indexTimeline = function () {
    ...
    var redEvents = [[new Date('1986/02/28 20:35:00'),, '<span class="timeline-text op-event-content">Lämnar bostaden</span>'], [new Date('1986/02/28 20:55:00'),, '<span class="timeline-text op-event-content">Anländer Grand</span>'], [new Date('1986/02/28 23:12:00'),, '<span class="timeline-text op-event-content">Lämnar Grand</span>'], [new Date('1986/02/28 23:21:00'),, '<span class="timeline-text op-event-content">Mordet</span>']];
    ...
    function drawVisualization() {
        ...
        if (index.checkLoadStatus() && !core.getPkStatus('timelineEvents')) {
            //if status ok and timeline events not added, add them and proceed
    ...
    function insertEvents() {
        ...
        if (obs !== null) {
            obs.forEach(function (element) {
                rows.push([dateToUTC(element.obsDate),, '<a href="/observation?id=' + element._id + '" class="observation-link timeline-text" data-id=' + element._id + '>' + element.title + '</a>']);
            }, this);
            if (core.getPkSettings('showStatic')) {
                rows.push.apply(rows, redEvents); //add static events w spread!
            }
